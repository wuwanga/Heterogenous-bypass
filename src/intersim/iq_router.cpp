      ...
      for ( int vc = 0; vc < _vcs; ++vc ) {
         ...
         if ( cur_vc->GetState( ) == VC::idle ) {
            ...
            if ( f ) {
               ...
               // UNT Test
               //printf("-----f->id:%d,f->src:%d,f->dest:%d, node:%d\n", f->id, f->src, f->dest, this->GetID());
   ...
   for ( int input = 0; input < _inputs; ++input ) {
      for ( int vc = 0; vc < _vcs; ++vc ) {
         ...
         if ( ( cur_vc->GetState( ) == VC::vc_alloc ) &&
            ...
            _AddVCRequests( cur_vc, input*_vcs + vc, f->FlitsSta, f->watch );
         }
      }
   }
