...
void dim_order_mesh( const Router *r, const Flit *f, int in_channel, OutputSet *outputs, bool inject )
{
   ...
   if ( inject ) { // use any VC for injection
      ...
      // UNT modified
      out_port = dor_next_mesh( r->GetID( ), f->dest, f->FlitsSta );
      ...
      //UNT added
      //Virtual channel partition
      //if ( r->GetID() == f->dest ) {
         outputs->AddRange( out_port, 0, gNumVCS - 1 );
      //} else {
         //if ( f->FlitsSta / 2 ) {
          //  outputs->AddRange( out_port, 0, _cpu_vcs - 1 );
         //} else {
            outputs->AddRange( out_port, 0, gNumVCS - 1 );
         //}
     // }
...
void romm_mesh( const Router *r, const Flit *f, int in_channel, OutputSet *outputs, bool inject )
{
   ...
   if ( f->ph == 1 ) { // In phase 1
      out_port = dor_next_mesh( r->GetID( ), f->intm, f->FlitsSta );
      vc_min = 0;
      vc_max = gNumVCS/2 - 1; 
   } else { // In phase 2
      out_port = dor_next_mesh( r->GetID( ), f->dest, f->FlitsSta );
   ...
   if ( inject ) {
      ...
      if ( cur != dest ) {
         if ( ( f->vc != gNumVCS - 1 ) && 
            ...
            outputs->AddRange( dor_next_mesh( cur, dest, f->FlitsSta ),
