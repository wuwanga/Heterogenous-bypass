          ...
          inputs, outputs )
{
   ...
   for( int j=0; j < _outputs; ++j ) {
      _output_state[j].init( config, seperate_network_ratio, net_num );
   }
...
void EventNextVCState::init( const Configuration& config, int seperate_network_ratio, int net_num )
{
   _Init( config, seperate_network_ratio, net_num );
}

EventNextVCState::EventNextVCState( const Configuration& config, 
                                    Module *parent, const string& name, int seperate_network_ratio, int net_num ) :
Module( parent, name )
{
   _Init( config, seperate_network_ratio, net_num );
}

void EventNextVCState::_Init( const Configuration& config, int seperate_network_ratio, int net_num )
{
