...
KNFly::KNFly( const Configuration &config, int seperate_network_ratio, int net_num ) :
Network( config )
{
   _ComputeSize( config );
   _Alloc( );
...
void KNFly::_BuildNet( const Configuration &config, int seperate_network_ratio, int net_num )
{
   ...
   for ( int stage = 0; stage < _n; ++stage ) {
      for ( int addr = 0; addr < per_stage; ++addr ) {
         ...
         _routers[node] = Router::NewRouter( config, this, router_name.str( ), 
                                             node, _k, _k, seperate_network_ratio, net_num );
